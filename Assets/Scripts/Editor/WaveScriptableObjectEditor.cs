...
[CustomEditor(typeof(WaveScriptableObject))]
[CanEditMultipleObjects]
public class WaveScriptableObjectEditor : Editor
{
    ...
    void OnEnable()
    {
        ...
        waitBeforeSpawn = serializedObject.FindProperty("waitBeforeSpawn");

        if (spawnChance == null || itemPrefab == null || waitBeforeSpawn == null)
        {
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField("field of a Wave Manager to edit");

        serializedObject.Update();

        WaveScriptableObject targetSpawnObject = target as WaveScriptableObject;
        ...
        if (targetSpawnObject != null)
        {
            ...
            EditorGUILayout.LabelField("Wave Amounts", "Object to spawn");
            ...
            if (targetSpawnObject.spawnChance != null)
            {
                ...
                for (int i = 0; i < targetSpawnObject.spawnChance.Length; ++i)
                {
                    ...
                    int iThree = EditorGUILayout.IntField(targetSpawnObject.spawnChance[i], GUILayout.Width(50f));
                ...
                //Make a column of the object prefab pickers
                EditorGUILayout.BeginVertical();

                for (int t = 0; t < targetSpawnObject.spawnChance.Length; ++t)
                {

                    EditorGUI.BeginChangeCheck();
                    float fOne = EditorGUILayout.FloatField(targetSpawnObject.waitBeforeSpawn[t], GUILayout.Width(50f));
                    if (EditorGUI.EndChangeCheck())
                    {
                        Undo.RecordObject(obj.targetObject, "Change wait time");
                        targetSpawnObject.waitBeforeSpawn[t] = fOne;
                    }
                }
                EditorGUILayout.EndVertical();
